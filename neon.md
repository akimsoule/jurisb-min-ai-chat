...
https://ep-xxx.apirest.c-2.us-east-2.aws.neon.build/dbname/rest/v1 3. Enable Neon Auth (for authentication)
Go to your project settings in Neon Console
Enable Neon Auth
Copy your Auth URL
Auth URL format:

https://ep-xxx.neonauth.c-2.us-east-2.aws.neon.build/dbname/auth 4. Configure Environment Variables
...
@import 'tailwindcss';
@import '@neondatabase/neon-js/ui/tailwind'; 2. Setup Provider
"use client"
...
const { data: session } = await client.auth.getSession();
client.auth.onAuthStateChange((event, session) => { /_ ... _/ });
...
@neondatabase/auth - Authentication adapters (can be used standalone)
@neondatabase/postgrest-js - PostgreSQL client (can be used standalone)
Resources
Neon Documentation
